...
/*this is my own idea for a growable array. an ArrayList is growable by doubling the size of the array and copying all the
 * elems (short for elements) of the original array into the new one. my array is growable by having the elems
 * in inner arrays of a 2d array. the 2d array starts with one inner array. the user chooses what size the inner arrays should be
 * (note: the user chooses what size all the inner arrays should be; the user isn't choosing how big each new inner array
 * should be. they're all the same.). every time the user wants to add an elem it is put in the inner array, and when the inner array becomes full then a new 2d array is created with the
 * capacity to hold one more inner array, and the full inner array, and a new, empty inner array is put into
 * the new 2d array.
 * in my growable array, elements aren't being copied like ArrayLists, rather just the references to inner arrays.
 * use MainForComparing to compare how many copies would be made using my growable array as opposed to when using an ArrayList.*/
public class GrowableArray<T> {
	...
	public void insert(int userIndex, T newElem) {
		...
		/*if newElem was placed at an index where there was another elem then shift over the elem that was at that index
		 * and all the elems after it.*/
	...
	public void delete(int userIndex) {
		...
		/*setting it to null is necessary even though the for loops i'm going to move over all the elems and its spot will be filled with
		 * the elem to its right. the reason is that if the elem being removed was the last one (as in lastElem) entered then the
		 * for loop won't run past it to shift an elem to replace it, because there isn't an elem to shift over into its place.*/
